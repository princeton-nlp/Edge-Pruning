#!/bin/bash -l
#SBATCH --job-name=instr_prune-fllama
#SBATCH --nodes=4 
...
WARMUP=5500
SEED=42
OUTPUT_DIR=./data/runs/fllama-instr-s${SEED}-elr${ELR}-llr${LLR}-relr${RELR}-rllr${RLLR}-es${EDGE_SPARSITY}-ns${NODE_SPARSITY}-t${TOTAL}/
...
src/prune/fllama_boolean_expressions_instr.py \
--report_to wandb \
--do_train \
