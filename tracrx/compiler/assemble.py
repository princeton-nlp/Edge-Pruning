...
import numpy as np
from tracrx.craft import bases
from tracrx.craft import transformers
from tracrx.craft import vectorspace_fns
from tracrx.transformer import encoder
from tracrx.transformer import model
...
class AssembledTransformerModel:
  ...
  def apply(self, tokens: List[bases.Value]) -> AssembledTransformerModelOutput:
    ...
    return AssembledTransformerModelOutput(
        ...
        unembedding_mtx=output.unembedding_mtx,
    )
...
def _make_embedding_modules(
  ...
  return EmbeddingModules(
      token_embed=token_embed, pos_embed=pos_embed, unembed=unembed_mod), res_to_out.matrix
...
def assemble_craft_model(
  ...
  def get_compiled_model():
    ...
    embed_modules, out_mtx = _make_embedding_modules(
